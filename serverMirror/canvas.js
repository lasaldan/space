var CanvasRenderer = function(targetSelector) {
    ...
    game.renderOrder.forEach(function(layerId) {
      game.renderLayers[layerId].forEach(function(sprite) {
        ...
        if(sprite.tiled) {
          ...
          renderer.drawSprite(sprite.image, (sprite.body)? sprite.body.x:sprite.x, (sprite.body)?sprite.body.y:sprite.y, (sprite.body)?sprite.body.rotation:sprite.rotation, sprite.scale, sprite.frame, sprite.com)
        }
      })
    })
    renderer.ctx.translate(-renderer.camera.x() - renderer.width/2, -renderer.camera.y() - renderer.height/2)
  ...
  renderer.drawSprite = function(img,x,y,rotation,scale,frame,com) {
    if(!scale) scale = 1
    ...
    renderer.ctx.drawImage(img, frame.x, frame.y, frame.width, frame.height, -width/2 + com.x, -width/2 + com.y, width, height )
    renderer.ctx.rotate(-rotation*Math.PI/180)
    renderer.ctx.translate(-x , -y)
  }
